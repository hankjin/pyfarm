...
static BOOL Bell_OnCreate(HWND hwnd, LPCREATESTRUCT lpCreateStruct)
{
	...
	SetPropW(hwnd, _P_BELL_48, (HANDLE)LoadImageW(m_hInstance, MAKEINTRESOURCEW(IDI_BELL_48), IMAGE_ICON, 48, 48, LR_DEFAULTCOLOR));
	SetPropW(hwnd, _P_BELL_32, (HANDLE)LoadImageW(m_hInstance, MAKEINTRESOURCEW(IDI_BELL_32), IMAGE_ICON, 32, 32, LR_DEFAULTCOLOR));
	SetPropW(hwnd, _P_BELL_24, (HANDLE)LoadImageW(m_hInstance, MAKEINTRESOURCEW(IDI_BELL_24), IMAGE_ICON, 24, 24, LR_DEFAULTCOLOR));
	SetPropW(hwnd, _P_BELL_16, (HANDLE)LoadImageW(m_hInstance, MAKEINTRESOURCEW(IDI_BELL_16), IMAGE_ICON, 16, 16, LR_DEFAULTCOLOR));
...
static void Bell_OnDestroy(HWND hwnd)
{
	...
	if(GetPropW(hwnd, _P_BELL_BRUSH))
		DeleteObject((HBRUSH)GetPropW(hwnd, _P_BELL_BRUSH));
}
