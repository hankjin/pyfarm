...
strFrame+='         onmouseover="style.color=\'#ff0000\'" onmouseout="style.color=\'#1478eb\'" title="當前日期时间">当前</span>&nbsp;&nbsp;<span style="cursor:hand" id=evaAllOK onmouseover="style.color=\'#ff0000\'" onmouseout="style.color=\'#1478eb\'"  onclick="parent.closeLayer()" title="关闭日历">关闭&nbsp;</span></td></tr>';
strFrame+='    </table></td></tr><tr ><td >';
strFrame+='        <table border=0 cellspacing=1 cellpadding=0 width=100%  bgcolor=#FFFFFF height=22 style="display:none">';
strFrame+='          <tr bgcolor="#63A3E9"><td id=bUseTimeLayer width=30  style="cursor:hand" title="點擊這里啟用/禁用时间"';
...
{
 if (arguments.length > 2){alert("對不起！傳入本控件的参数太多！");return;}
 if (arguments.length == 0){alert("對不起！您沒有傳回本控件任何参数");return;}
...
{
 if (strYear.match(/\D/)!=null){alert("年份輸入参数不是數字！");return;}
...
{
 if (strMonth.match(/\D/)!=null){alert("月份輸入参数不是數字！");return;}
...
function tmpSelectHourInnerHTML(strHour) //小时的下拉框
{
 ...
 if (strHour.match(/\D/)!=null){alert("小时輸入参数不是數字！");return;}
...
function tmpSelectMinuteInnerHTML(strMinute) //分钟的下拉框
{
 ...
 if (strMinute.match(/\D/)!=null){alert("分鐘輸入數字不是數字！");return;}
...
function tmpSelectSecondInnerHTML(strSecond) //秒的下拉框
{
 ...
 if (strSecond.match(/\D/)!=null){alert("秒鐘輸入参数不是數字！");return;}
