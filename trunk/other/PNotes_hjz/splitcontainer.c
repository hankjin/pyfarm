...
HWND CreateMainPanel(int splitStyle, HINSTANCE hInstance, HWND hParent, HCURSOR hCursor, int nStopper, int nLeft, int nTop, int nRight, int nBottom){
	...
	if(hwnd){
		...
		switch(splitStyle){
			case SPLIT_HORZ:
				pData->cTop.hwnd = CreateWindowExW(0, SPLIT_SUB_PANEL_CLASS, NULL, WS_CHILD | WS_VISIBLE, 0, 0, width, height / 2 - 2, hwnd, NULL, hInstance, NULL);
				SetPropW(pData->cTop.hwnd, PROP_VISIBLE, (HANDLE)TRUE);
				pData->cBottom.hwnd = CreateWindowExW(0, SPLIT_SUB_PANEL_CLASS, NULL, WS_CHILD | WS_VISIBLE, 0, height / 2 + 2, width, height / 2 - 2, hwnd, NULL, hInstance, NULL);
				SetPropW(pData->cBottom.hwnd, PROP_VISIBLE, (HANDLE)TRUE);
				break;
			case SPLIT_VERT:
				pData->cLeft.hwnd = CreateWindowExW(0, SPLIT_SUB_PANEL_CLASS, NULL, WS_CHILD | WS_VISIBLE, 0, 0, width / 2 - 2, height, hwnd, NULL, hInstance, NULL);
				SetPropW(pData->cLeft.hwnd, PROP_VISIBLE, (HANDLE)TRUE);
				pData->cRight.hwnd = CreateWindowExW(0, SPLIT_SUB_PANEL_CLASS, NULL, WS_CHILD | WS_VISIBLE, width / 2 + 2, 0, width / 2 - 2, height, hwnd, NULL, hInstance, NULL);
