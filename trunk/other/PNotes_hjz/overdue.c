...
static BOOL Overdue_OnInitDialog(HWND hwnd, HWND hwndFocus, LPARAM lParam)
{
	...
	m_hSplitter = CreateMainPanel(SPLIT_HORZ, g_hInstance, hwnd, g_CurH, 36, 0, 0, 0, cH);
	...
	for(int i = 0; i < g_OverdueCount; i++){
		...
		//if(parr[i]->pSchedule->scType == SCH_ONCE || parr[i]->pSchedule->scType == SCH_AFTER){
			...
			//if(g_hCPDialog){
				...
				//SendMessageW(g_hCPDialog, CHM_CTRL_ITEM_UPDATE, 0, (LPARAM)parr[i]);
			//}
		//}
	}
}
...
static void ApplyOverdueLanguage(void){
	...
	GetPrivateProfileStringW(S_OPTIONS, szKey, L"Overdue reminders", szBuffer, 256, szFile);
	...
	GetPrivateProfileStringW(S_COLUMNS, L"0", L"Note Name", szBuffer, 256, szFile);
	SendMessageW(m_hList, LVM_SETCOLUMNW, 0, (LPARAM)&lvc);
	SendMessageW(m_hList, LVM_GETCOLUMNW, 1, (LPARAM)&lvc);
	GetPrivateProfileStringW(S_COLUMNS, L"3", L"Schedule Type", szBuffer, 256, szFile);
	SendMessageW(m_hList, LVM_SETCOLUMNW, 1, (LPARAM)&lvc);

	SetDlgCtlText(g_hOverdueDialog, IDOK, szFile, L"OK");
}
