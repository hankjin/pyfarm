...
inline double crossproduct(Point start,Point U,Point V){//¿¿¿¿
...
inline bool watch(Point start,Point end){
    if(sqrt(((start.x-end.x)*(start.x-end.x))+((start.y-end.y)*(start.y-end.y)))<JD) 
	return false;
    Line mid;//¿¿¿
    bool temp=false;
...
int main()
{
    ...
    for(int i=1;i<n;i++)
	for(int j=i+1;j<=n;j++)
	    if(cancross(line[i],line[j])==1){
		fout<<"NOFENCE"<<endl;
		return 0;
	    }
    for(int i=1;i<=n;i++){
	if(crossproduct(see,line[i].start,line[i].end)==0) // . --------
	    continue;
	reg[i]=true;//
	if(watch(line[i].start,line[i].end)){
	    len++; 
	    cansee[i]=true;
	}
	reg[i]=false;
    ...
    fout<<len<<endl;
    if(cansee[n]&&cansee[n-1]){ //strange sequence
	Line temp=line[n];
	line[n]=line[n-1];
	line[n-1]=temp;
    }
